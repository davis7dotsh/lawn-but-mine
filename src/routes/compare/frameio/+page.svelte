...
<svelte:head>
  <title>lawn vs Frame.io — the cheaper, faster alternative</title>
  <meta name="description" content="Compare lawn and Frame.io. Flat $5/month pricing vs per-seat billing. Unlimited seats, instant playback, open source. See why teams are switching." />
</svelte:head>
    ...
    <div class="max-w-5xl mx-auto">
      ...
      <div class="border-2 border-[var(--border)] shadow-[8px_8px_0px_0px_var(--shadow-color)] bg-[var(--background)]">
        <div class="grid grid-cols-3 border-b-2 border-[var(--border)] bg-[var(--surface-strong)] text-[var(--foreground-inverse)]">
          ...
          <div class="p-4 md:p-6 font-black uppercase tracking-wider text-sm border-l-2 border-[var(--border)]">Frame.io</div>
        ...
        {#each comparisonRows as row, i}
          <div class="grid grid-cols-3 {i < comparisonRows.length - 1 ? 'border-b-2 border-[var(--border)]' : ''}">
            ...
            <div class="p-4 md:p-6 border-l-2 border-[var(--border)] flex items-center text-[var(--foreground-muted)] font-medium">{row.frameio}</div>
            <div class="p-4 md:p-6 border-l-2 border-[var(--border)] flex items-center font-bold text-[var(--accent)]">{row.lawn}</div>
          </div>
        {/each}
      </div>
      <p class="text-center text-sm text-[var(--foreground-muted)] mt-6 md:hidden">* Frame.io pricing based on their Team plan at $19/user/month.</p>
    </div>
  </section>
        ...
        {#each teamSizes as size}
          ...
          <div class="border-2 border-[var(--border)] bg-[var(--background)] shadow-[6px_6px_0px_0px_var(--shadow-color)] hover:-translate-y-1 hover:translate-x-1 hover:shadow-[4px_4px_0px_0px_var(--shadow-color)] transition-all flex flex-col">
            <div class="border-b-2 border-[var(--border)] bg-[var(--surface-strong)] text-[var(--foreground-inverse)] p-5">
              ...
              <span class="text-sm font-bold uppercase tracking-wider text-[var(--foreground-muted)] ml-2">{size === 1 ? "person" : "people"}</span>
            </div>
            <div class="p-5 flex flex-col flex-grow">
              <div class="flex justify-between items-baseline mb-1">
                <span class="text-xs font-bold uppercase tracking-wider text-[var(--foreground-muted)]">Frame.io</span>
                <span class="font-black text-[var(--foreground-muted)] line-through">${frameioAnnual.toLocaleString()}/yr</span>
              </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div class="border-2 border-[var(--border)] bg-[var(--background)] shadow-[8px_8px_0px_0px_var(--shadow-color)]">
          <div class="border-b-2 border-[var(--border)] p-6">
            <h3 class="text-2xl md:text-3xl font-black uppercase tracking-tighter">Use Frame.io if...</h3>
          </div>
          <div class="p-6">
            <ul class="space-y-5">
              <li class="flex items-start gap-3"><span class="text-[var(--foreground-muted)] font-black text-lg shrink-0 mt-0.5">--</span><span class="font-medium">You need enterprise compliance docs (SOC 2, etc.) for your procurement team to approve anything</span></li>
              <li class="flex items-start gap-3"><span class="text-[var(--foreground-muted)] font-black text-lg shrink-0 mt-0.5">--</span><span class="font-medium">You're deeply embedded in Adobe Premiere and After Effects and need native panel integration</span></li>
              <li class="flex items-start gap-3"><span class="text-[var(--foreground-muted)] font-black text-lg shrink-0 mt-0.5">--</span><span class="font-medium">You have 100+ people with complex multi-stage approval workflows and version trees</span></li>
              <li class="flex items-start gap-3"><span class="text-[var(--foreground-muted)] font-black text-lg shrink-0 mt-0.5">--</span><span class="font-medium">Budget isn't a concern and you want every feature imaginable, even the ones you'll never use</span></li>
            </ul>
            <p class="text-sm text-[var(--foreground-muted)] mt-6 pt-4 border-t-2 border-[var(--border-subtle)]">Genuinely, Frame.io is solid software. If this is you, go use it. We won't be offended. (Okay maybe a little.)</p>
          </div>
        </div>
        <div class="border-2 border-[var(--border)] bg-[var(--surface-strong)] text-[var(--foreground-inverse)] shadow-[8px_8px_0px_0px_var(--shadow-accent)]">
          ...
          <div class="p-6">
            <ul class="space-y-5">
              <li class="flex items-start gap-3"><span class="text-[var(--accent-light)] font-black text-lg shrink-0 mt-0.5">--</span><span class="font-medium">You're a small-to-mid team that just needs to share cuts and collect feedback without a NASA control panel</span></li>
              <li class="flex items-start gap-3"><span class="text-[var(--accent-light)] font-black text-lg shrink-0 mt-0.5">--</span><span class="font-medium">You're an agency tired of doing per-seat multiplication every time you onboard a client</span></li>
              <li class="flex items-start gap-3"><span class="text-[var(--accent-light)] font-black text-lg shrink-0 mt-0.5">--</span><span class="font-medium">You're a freelancer who just needs to show a cut to a client without making them create yet another account</span></li>
              <li class="flex items-start gap-3"><span class="text-[var(--accent-light)] font-black text-lg shrink-0 mt-0.5">--</span><span class="font-medium">You value speed and simplicity over a feature checklist that makes the marketing site look impressive</span></li>
            </ul>
            <p class="text-sm text-[var(--foreground-muted)] mt-6 pt-4 border-t border-[#333]">We do less than Frame.io. Proudly. Turns out "upload, share, comment" is 90% of what anyone actually needs.</p>
          </div>
        </div>
      </div>
    </div>
  ...
  <section class="px-6 py-32 bg-[var(--background)]">
    <div class="max-w-4xl mx-auto text-center flex flex-col items-center">
      ...
      <p class="text-sm text-[var(--foreground-muted)] mt-6">
        Or keep paying $19/user/month. We don't judge.<br />(We judge a little.)
      </p>
    </div>
  </section>
</MarketingLayout>
